...
class LighteningFileBufferTest < Test::Unit::TestCase
  BUFFER_PATH = 'dummy'
  CONFIG = %[
    buffer_type lightening_file
    flush_interval 0.1
    try_flush_interval 0.03
    buffer_chunk_records_limit 10
    buffer_path #{BUFFER_PATH}/test
  ]
  ...
  def teardown
    require 'fileutils'
    FileUtils.rm_r(BUFFER_PATH) if File.exists? BUFFER_PATH
  end
end
